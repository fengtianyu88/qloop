...
本文档是对 v1.3.0 设计文档的增量补充,覆盖 v1.4.0 → v1.4.6 期间已落地的所有新功能与 Bug 修复。**原设计文档中的所有内容仍然有效**,除非本文档明确声明覆盖。
...
## 一、文档解析器 ZIP 解压与多格式支持(v1.4.6 修复)
...
## 七、LLM 流式输出(v1.4.2 引入,v1.4.6 确认)
...
## 八、评审失败特批放行(v1.4.2 引入,v1.4.6 确认)
...
### 10.2 `notifications` 表(已存在,v1.4.6 开始有数据)
