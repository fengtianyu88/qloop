...
## 第 18 章 v1.5.0 — 系统测试 + 2 个产品 Bug 修复（2026-07-23）

### 18.1 v1.5.0 概述

v1.5.0 是一次以**全面系统测试**为核心的质量保障版本。基于 v1.4.7 的最新设计文档，采用 7 大测试设计方法（等价类划分、边界值分析、决策表、状态迁移法、场景法、错误推测法、异常分析法）设计了 **160 个测试用例**，覆盖 16 个功能模块。经过 8 轮测试迭代（4 轮主测试 + 4 轮补充/重测），最终实现 **160/160 全部通过，0 缺陷遗留**。

测试过程中发现并修复了 **2 个产品 Bug**：
- **BUG-01（高严重度）**：MinIO 预签名 URL 有效期违反 7 天硬约束
- **BUG-02（中严重度）**：空文件上传未拦截

### 18.2 修复的产品 Bug

#### BUG-01：MinIO presigned URL 有效期错误

| 项 | 内容 |
|----|------|
| 严重度 | 高 |
| 模块 | 下载链路 |
| 现象 | `backend/app/api/releases.py` 中两个下载端点使用 `expiry_hours=1`（1 小时），违反项目硬约束"MinIO 预签名 URL 有效期 7 天（168 小时）" |
| 影响 | 用户分享的下载链接 1 小时后失效，无法满足交付窗口需求 |
| 修复 | `expiry_hours=1` → `expiry_hours=168`（7 天），同时更新 docstring |
| 验证用例 | TC-DL-06：X-Amz-Expires=604800（7 天） |
| 文件 | `backend/app/api/releases.py` L434、L505、L409 docstring |
...
| 严重度 | 中 |
| 模块 | 上传链路 |
| 现象 | `backend/app/services/release_service.py` 中 3 个上传函数（`upload_code_package`、`upload_test_report`、`upload_review_report`）缺少空文件检查，上传空文件返回 HTTP 200 |
| 影响 | 攻击者可上传空文件占位，污染数据库与 MinIO 存储 |
| 修复 | 在文件类型校验之前添加 `if not file_data: raise ValueError("文件内容为空,请上传有效的文件")` |
| 验证用例 | TC-UPLOAD-14：上传空文件返回 415 |
| 文件 | `backend/app/services/release_service.py`（3 处） |
...
| HC（健康检查） | 2 | 2 | 0 | 100% |
| PROJ（项目管理） | 8 | 8 | 0 | 100% |
| STATE（状态机） | 14 | 14 | 0 | 100% |
| FORCE（特批放行） | 6 | 6 | 0 | 100% |
| NOTIF（通知系统） | 16 | 16 | 0 | 100% |
| DL（下载链接） | 6 | 6 | 0 | 100% |
| FE（前端） | 8 | 8 | 0 | 100% |
| SEC（安全） | 8 | 8 | 0 | 100% |
| PAGE（分页） | 5 | 5 | 0 | 100% |
| EXC（异常处理） | 6 | 6 | 0 | 100% |
| UPLOAD（上传） | 15 | 15 | 0 | 100% |
| LLM（LLM 评审） | 16 | 16 | 0 | 100% |
| SSE（SSE 流式） | 10 | 10 | 0 | 100% |
| PERM（权限矩阵） | 8 | 8 | 0 | 100% |
| LLM-CONFIG（LLM 配置） | 4 | 4 | 0 | 100% |
| **合计** | **160** | **160** | **0** | **100%** |
...
| 轮次 | 范围 | 通过 | 失败 | 跳过 | 通过率 |
|------|------|------|------|------|--------|
| 第一轮 | 47 项主测试 | 30 | 10 | 7 | 63.8% |
| 第二轮 | 113 项完整测试 | 64 | 18 | 31 | 56.6% |
| 第三轮 | 113 项修复后重测 | 84 | 2 | 27 | 74.3% |
| 第四轮 | 113 项二次重测 | 87 | 0 | 26 | 77.0% |
| 补充 | 28 项跳过用例 | 25 | 3 | 0 | 89.3% |
| 重测 | 3 项失败用例 | 3 | 0 | 0 | 100% |
| P0 补充 | 47 项未覆盖用例 | 46 | 1 | 0 | 97.9% |
| LLM-08 重测 | 1 项 | 1 | 0 | 0 | 100% |
| **最终合并** | **160 项** | **160** | **0** | **0** | **100%** |

### 18.5 关键测试验证

#### LLM 评审真实测试

- **模型**：MiniMax-M3（OpenAI 兼容协议）
- **触发**：`POST /api/reviews/trigger/{release_id}?review_type=code_review`
- **结果**：返回 202，task_id 创建，状态 PENDING → DONE
- **评审输出**：total_score=52.0, result=passed, model_used=MiniMax-M3
- **结论**：包含代码规范、文档、测试覆盖等维度，真实可用

#### SSE 流式输出

- **端点**：`GET /api/reviews/stream/{release_id}?token=xxx`
- **事件类型**：step（步骤进度）+ chunk（LLM 流式输出）+ done（完成事件）
- **真实测试结果**：10 个 step 事件 + 143 个 chunk 事件，LLM 输出实时推送
- **修复的 Bug**：SSE 优先查找 PENDING 评审 + 先订阅 Redis 再查 DB，避免推送旧 done

#### 权限矩阵验证

覆盖 super_admin / admin / project_manager / developer / tester / expert_reviewer 六个角色，验证：
- 系统设置仅 super_admin 可改（TC-SEC-05/06/07/08）
- 项目成员管理：super_admin/admin 修改所有成员，PM 仅能修改非 PM 成员（TC-PERM-01~08）
- 版本删除：super_admin 可删除已发布版本，admin 仅能删除未发布版本（TC-STATE-12~14）
...
包括 REVIEW_FAILED 分支和特批放行路径。

### 18.6 测试交付物

| 交付物 | 路径 |
|--------|------|
| 测试用例 Excel（160 用例） | `docs/tests/v1.5.0/QLoop_v1.5.0_测试用例.xlsx` |
| 测试报告 | `docs/tests/v1.5.0/QLoop_v1.5.0_测试报告.md` |
| 合并测试结果 JSON | `docs/tests/v1.5.0/merged_test_results.json` |
| 测试脚本 | `docs/tests/v1.5.0/scripts/`（含 14 个 Python 脚本） |

### 18.7 测试方法论

采用 7 大测试设计方法确保全面性：

1. **等价类划分**：将输入域划分为有效/无效等价类（如密码长度、文件类型）
2. **边界值分析**：测试边界值（如密码 7/8 字符、page_size=0/1/100/1000）
3. **决策表**：多条件组合场景（如权限矩阵 × 操作类型）
4. **状态迁移法**：Release 状态机迁移路径全覆盖
5. **场景法**：端到端业务流程（如 4 角色完整释放流程）
6. **错误推测法**：基于经验推测错误（如 SQL 注入、空请求体）
7. **异常分析法**：异常路径覆盖（如 404/422/500 错误响应）

### 18.8 测试结论

1. **160 个测试用例全部通过，0 缺陷遗留**
2. **发现并修复 2 个产品 Bug**（presigned URL 有效期 + 空文件上传）
3. **LLM 评审真实测试通过**，MiniMax-M3 模型真实调用，返回 score=52.0
4. **SSE 流式输出真实测试通过**，10 个 step + 143 个 chunk 事件
5. **16 个模块全部覆盖**，权限矩阵 / 状态机 / 异常处理 / 安全防护全部验证

**重要声明**：测试覆盖率"100%"是相对于当前定义的 160 个测试用例集合而言的。真实测试过程中发现了 2 个产品 Bug，证明测试是有效的。软件测试不能证明"没有问题"，只能证明"已测试的场景没有问题"。未来可能随着使用场景增加而发现新的 bug。

### 18.9 文件变更清单

| 文件 | 变更 |
|------|------|
| `backend/app/api/releases.py` | L434/L505: `expiry_hours=1` → `168`; L409 docstring 更新 |
| `backend/app/services/release_service.py` | 3 处上传函数添加空文件检查 |
| `README.md` | 版本号 1.4.7 → 1.5.0; 新增 v1.5.0 changelog 行 |
| `README_zh-CN.md` | 版本号 1.4.7 → 1.5.0; 新增 v1.5.0 changelog 行 |
| `docs/DEPLOYMENT.md` | 版本号 1.4.7 → 1.5.0; 日期 2026-07-23 |
| `backend/app/main.py` | FastAPI 应用版本 + /api/health 接口版本 → 1.5.0 |
